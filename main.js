...
class DbCreator {
    constructor(dbUrl) {
        this.MetaModel = mongoose.model('Meta', new mongoose.Schema({ lastUpdated: Date }));
        this.MutualFundSchema = new mongoose.Schema({
        ...
        this.MutualFundSchema.index({ 'Scheme Name': 'text' });
        this.MutualFund = mongoose.model('MutualFund', this.MutualFundSchema);
        this.dbUrl = dbUrl;
        this.ms1Day = 24 * 60 * 60 * 1000;
    }

    static async getInstance(dbUrl) {
        if (!this.instance) {
            await mongoose.connect(dbUrl);
            this.instance = new DbCreator(dbUrl);
        }
    ...
    async createMeta() {
        var meta = await this.MetaModel.create({
    ...
    async getLastUpdated() {
        const metas = await this.MetaModel.find();
        var meta = metas[0];
        if (meta === undefined)
    ...
    async persistData(data, updateDate) {
        ...
        for (let item of uniques) {
            ...
            await this.MutualFund.updateOne({ 'Scheme Code': item['Scheme Code'] }, {
                ...info,
                $addToSet: {
                    'NAVs': { $each: NAVs }
                }
